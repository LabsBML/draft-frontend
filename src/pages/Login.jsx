...
const loginSchema = zod.object({
  email: zod.string().min(1, 'Email validation index requires a targeted address entry.').email('Provide a structurally accurate email address.'),
  password: zod.string().min(6, 'Security credentials must contain at least 6 structural units.'),
});
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setServerError(response.message || 'Verification rejected by processing cluster.');
      }
    } catch (err) {
      setServerError(err.message || 'Authentication framework connection exception.');
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        ...
        <div className="flex items-center justify-between mb-6 border-b border-neutral-100 pb-4">
          ...
          <div className="text-right">
            <h2 className="text-[16px] font-semibold text-[#111111] tracking-tight">Welcome back</h2>
            <p className="text-[12px] text-neutral-400">Sign in to your application</p>
          </div>
        </div>

        {/* Global Server Processing Errors Banner */}
        {serverError && (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3.5">
            ...
            {/* Column 2: Security Key / Password */}
            <div className="space-y-1">
              <label className="block text-[13px] font-medium text-[#111111]">Security Key</label>
              <Input 
                type="password" 
                placeholder="••••••••"
          ...
          {/* Utility Configuration Links Row */}
          <div className="pt-1 flex items-center justify-between">
            <label className="flex items-center gap-2 cursor-pointer select-none">
              ...
              />
              <span className="text-[12px] text-neutral-500 font-normal">Remember me</span>
            </label>
            <a 
              href="#forgot" 
              className="text-[12px] text-[#111111] hover:underline font-medium transition-all duration-150"
            >
              Forgot password?
            </a>
          </div>
